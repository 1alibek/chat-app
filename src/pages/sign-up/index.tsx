  ...
  return (
    <section className="h-screen grid lg:grid-cols-2">
      <div className="flex items-center justify-center flex-col p-6 sm:p-12">
        ...
        <form onSubmit={handleSubmit} className="w-[75%] max-[500px]:w-[100%] flex flex-col gap-5">
          ...
          <div className="form-control">
            ...
            <div className="flex justify-between border p-[2px_10px] rounded-lg items-center">
              ...
              <div onClick={() => setShowPass(!showPass)} className="cursor-pointer h-10 px-1 flex justify-center items-center">
                {showPass ? <EyeOff className="h-5 w-5 text-base-content/40" /> : <Eye className="h-5 w-5 text-base-content/40" />}
              </div>
            </div>
          </div>

          <button disabled={isSigninLoading} type="submit" className="btn btn-primary w-full">
            {isSigninLoading ? (
              <>
