...
export const useAuthStore = create<AuthType>((set) => ({
  ...
  checkUser: async () => {
    ...
    try {
      ...
      set({ authUser: res.data.data, isCheckingUserLoader: false });
    } catch (error) {
      ...
      set({ authUser: null, isCheckingUserLoader: false }); // Tekshiruv tugagach loader o'chiriladi
    }
  },
  ...
  signIn: async (data) => {
    ...
    try {
      ...
      set({ authUser: res.data.user, isLoginLoading: false });
